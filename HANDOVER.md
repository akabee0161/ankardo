...
設計は `docs/superpowers/specs/2026-08-25-site-header-footer-and-brand-design.md` に確定済み。段階②から意図的に外して持ち越す作業: **既存ボタン（`bg-neutral-900`）の配色をブランド色の藍に寄せるかの判断**、**アクセス解析の導入**（about の文言が制約になる）。実装前にユーザーからの入力が必要なもの: **Google フォームの URL**。
